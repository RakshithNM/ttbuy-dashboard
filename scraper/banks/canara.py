...
def parse(html, source_url):
    ...
    TT/CHQ, i.e. the first column of the BUYING RATES group (index 3).

    The page also has a separate 'International Prepaid' cash-rate table with
    its own USD row; restrict to the table containing 'TT/CHQ' so that one is
    never mistaken for the card-rate table.
    """
    ...
    for table in soup.find_all("table"):
        ...
        for row in table.find_all("tr"):
            ...
            if len(cells) <= 3 or not cells[0].upper().startswith("USD"):
                continue
